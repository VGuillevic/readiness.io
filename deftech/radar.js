	...
	if (window.sessionStorage.getItem('login') != themes[theme].client ||
		window.sessionStorage.getItem('password') != themes[theme].pw ){ 
...
window.onload = function (){   
	...
	function toggle_setting(){
		...
		if(this.on){
			this.on = false;
			this.style.color = themes[theme].med;
			this.style.background = themes[theme].drk;
			this.innerHTML = "OFF";
	...
	function toggle_starred(){ 
		...
		if(settings_bt3.on){
			...
			settings_bt3.style.color = themes[theme].med;
			settings_bt3.style.background = themes[theme].drk;
	...
	function open_filter (){ 
		...
		categ_lb.innerHTML = "CHOOSE A CATEGORY";
							 ...
							 for( b in json.startups ){ 
								 ...
								 if( __node != cur_node && 
									 __node.connected.indexOf( _node.id ) >= 0 &&
								   	 __node.visible) {	 
									 	connect( _node, __node, .3, themes[theme].med );
										startup_on(__node, themes[theme].med, white, white ); 
								 } 
							 }
	...
	function set_location(){  
		...
		else new_loc = location + "?categ=" + cur_categ + "&" + list_lbs[cur_list][1] + "=" + cur_node.id + "&slider=" + slider_x;
					...
					if( list_id == 1 ){  
						...
						for( a in json.startups ){ 
							...
							if( _node.connected.indexOf( node.id ) >= 0 ) { 
								startup_on(_node, themes[theme].med, white, white ); 
							}else{ 	  
								startup_off(_node); 
							}
						}  
						...
						for( a in json.techs ){
							...
							if( node.connected.indexOf( _node.id ) >= 0 ) {	 
								...
								tech_on(_node, themes[theme].med, themes[theme].med, 1 ); 
	...
	function startup_off(node){ 
		node.container.attr( 'stroke', themes[theme].drk ); 
		node.container.attr('fill', themes[theme].med); 
		node.star.attr('fill', themes[theme].med); 
	}
	...
	function tech_off(node){
		node.circle.attr( 'fill', themes[theme].drk );
		node.circle.attr( 'fill-opacity', op_up );
		node.label.attr( 'fill', themes[theme].drk );
	}
	...
	function reset_techs(){
		for ( i in json.techs ){ 
			node = json.techs[i];
			if (node.visible) tech_on(node, themes[theme].med, themes[theme].med, op_up );
		}	
	}
	...
	function reset_startups(){
		for( i in json.startups ){ 
			...
			if (node.visible) startup_on(node, themes[theme].brg, white, drk_gray);
		}	
	}
	...
	function check_get( push ){ 
		...
		if( !get_categ ){
			...
			if ( get_categ != cur_categ ){
				set_categ( get_categ, true );	
				delay = dur2;
			}else{
				delay = 0;				
				categ.style.zIndex = "";	
	...
	map_techs.append( 'circle' )
		...
		.attr('stroke', themes[theme].drk )
		.attr('stroke-width', .1)
		.attr('stroke-opacity',.5)
		.attr('fill', themes[theme].drk)
		.attr('fill-opacity', .3) 
	...
	d3.json("data_base.json", function(error, data)  { 
		...
		json.techs.forEach(function(d,i) { 
			...
			itm_mask.src = "_layout/mask_techs.png"; 
	...
	curtain.style.background = themes[theme].bg;  
	...
	slider_circle.style.background = themes[theme].brg; 
	
	settings_bt2.style.color = themes[theme].med;
	settings_bt2.style.background = themes[theme].drk;
	
	$(settings_user).hide();
	
	logo_client.src = "_layout/logo_" + themes[theme].client + ".png";
